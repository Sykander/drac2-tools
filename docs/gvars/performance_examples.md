...
- **`bench_counter_plus_assign` / `bench_counter_plus_eq`** — **`i = i + 1`** vs **`i += 1`**; optional second argument **`reps_per_iter`** (or **`-reps`** on the perf alias) repeats that many increments per outer step to stress both increment styles beyond Draconic’s default **`max_loops`** plateau at **`reps` 1**.
