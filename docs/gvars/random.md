...
A helper library for seeded random integers: you get deterministic draws when you control the seed, and optional one-off seeds that do not advance global state.
...
Runs one xorshift step on `seed`, reduced modulo `2**31`; used by `get_random_integer`.
