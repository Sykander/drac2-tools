...
Project rules in **`.cursor/rules/`** spell this out: **`drac2-tools-maintainer.mdc`** (always on: docs, tests, **`unused_gvars.md`** / workshop UUID hygiene), **`drac2-avrae-sources.mdc`** (Drac2 file roles, test vs production aliases, cached Avrae RST under **`.cursor/avrae-reference/`**), and **`gvar-perf-boundaries.mdc`** when tuning **`*-perf`** stress tests or **`utils/probe*.py`** boundary scripts.

## Tooling

- **Drac2 tests:** `avrae-ls --run-tests src` (see `.github/workflows/test.yaml`).
- **Perf stress boundaries (any util):** Re-tune **`*-perf.alias-test`** with a **single-test probe file** + **cap / gallop / binary** search (see **`utils/probe_regex_perf_boundaries.py`** as the regex reference). Workflow and pitfalls: **`.cursor/rules/gvar-perf-boundaries.mdc`**.
